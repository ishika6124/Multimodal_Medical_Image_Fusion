"""
CT-MRI Fusion — Combined Training + Inference Pipeline v2
==========================================================
Changes from v1:
  - Redesigned APFM: dual independent gates (CT + MRI no longer zero-sum)
  - Full spatial attention at patch resolution for each modality
  - Residual connection in APFM to prevent feature/signal collapse
  - Fixed DynamicModalityLoss (balanced CT/MRI contribution)
  - Gate balance penalty in training loop
  - CT edge preservation in GradientLoss
  - Extra CT SSIM penalty in FusionLoss

Architecture:
  - Frozen MAE ViT-Small encoders (CT + MRI), patch=16, embed=384, img=256
  - Dual-gate APFM fusion at every encoder layer
...
Usage:
    python fusion_pipeline_v2.py --mode train
    python fusion_pipeline_v2.py --mode infer --ckpt /path/to/best_model.pth
    python fusion_pipeline_v2.py --mode both
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1.  APFM  — Dual Independent Gate (v2)
# ─────────────────────────────────────────────────────────────────────────────
class APFM(nn.Module):
    """
    Dual-gate Adaptive Pooling Fusion Module.

    Key improvements over v1:
      - Independent gates for CT (x) and MRI (y): not zero-sum, both can
        contribute fully in their respective informative regions.
      - Spatial attention at full feature resolution: tells WHERE each
        modality is informative, not just which channels.
      - Residual from mean input: prevents signal/entropy collapse.

    Interface unchanged: forward(x, y) -> fused tensor, same shape.
    """
    def __init__(self, in_channels: int):
        ...
        # Shared context: understands joint CT+MRI content
        ...
        # Independent channel attention: which features matter per modality
        self.ct_channel_gate = nn.Sequential(
        ...
        self.mri_channel_gate = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(in_channels, mid, 1),
            nn.SiLU(),
            nn.Conv2d(mid, in_channels, 1),
        ...
        # Independent spatial attention: where each modality is informative
        self.ct_spatial_gate = nn.Sequential(
            nn.Conv2d(in_channels * 2, in_channels // 2, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(in_channels // 2, 1, 1),
            nn.Sigmoid()
        )
        self.mri_spatial_gate = nn.Sequential(
            nn.Conv2d(in_channels * 2, in_channels // 2, 3, padding=1),
            nn.SiLU(),
            nn.Conv2d(in_channels // 2, 1, 1),
            nn.Sigmoid()
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        """
        x : CT  features  (B, C, H, W)
        y : MRI features  (B, C, H, W)
        returns fused     (B, C, H, W)
        """
        xy     = torch.cat([x, y], dim=1)       # (B, 2C, H, W)
        shared = self.shared_conv(xy)            # (B,  C, H, W)

        # Independent channel gates
        ct_ch  = self.ct_channel_gate(shared)    # (B, C, 1, 1)
        mri_ch = self.mri_channel_gate(shared)   # (B, C, 1, 1)

        # Independent spatial gates
        ct_sp  = self.ct_spatial_gate(xy)        # (B, 1, H, W)
        mri_sp = self.mri_spatial_gate(xy)       # (B, 1, H, W)

        # Apply channel x spatial attention independently
        ct_attended  = x * ct_ch  * ct_sp        # CT  contribution map
        mri_attended = y * mri_ch * mri_sp       # MRI contribution map

        # Fuse by addition — no zero-sum constraint
        fused = ct_attended + mri_attended

        # Output projection
        out = self.out_proj(fused)

        # Residual from mean input: preserves signal energy, prevents collapse
        residual = 0.5 * (x + y)
        out = out + residual

        # Track CT spatial gate mean for balance monitoring (~0.5 is ideal)
        self.last_gate_mean = (
            ct_sp.mean() / (ct_sp.mean() + mri_sp.mean() + 1e-8)
        ).detach()
...
class CTMRIFusionModel(nn.Module):
    ...
    per-layer dual-gate APFM fusion (12 layers) ->
...
class FusionDataset(Dataset):
    ...
    def __init__(self, ct_root: str, mri_root: str, split: str = 'train'):
        ...
        ct_patients  = sorted(p.name for p in self.ct_root.iterdir()
                               if p.is_dir())
        mri_patients = sorted(p.name for p in self.mri_root.iterdir()
                               if p.is_dir())
...
# ─────────────────────────────────────────────────────────────────────────────
# 6.  Loss  — Balanced CT/MRI contribution
# ─────────────────────────────────────────────────────────────────────────────
class GradientLoss(nn.Module):
    """Sobel gradient loss with explicit CT-edge preservation."""
    def __init__(self):
        super().__init__()
        kx = torch.tensor(
            [[1, 0, -1], [2, 0, -2], [1, 0, -1]],
    ...
    def forward(self, pred, ref_ct, ref_mri):
        ...
        # Fused should match sharpest edge from either modality
        g_ref      = torch.max(gct, gmri)
        l_combined = F.l1_loss(gp, g_ref)

        # Preserve CT-specific edges (bones, calcifications)
        ct_only   = F.relu(gct - gmri)
        mask      = (ct_only > 0.01).float()
        l_ct_edge = (F.l1_loss(gp * mask, gct * mask)
                     if mask.sum() > 0 else gp.new_tensor(0.))
...
class DynamicModalityLoss(nn.Module):
    """
    Balanced independent losses.
    Each term scaled by the other's magnitude so neither CT nor MRI
    can dominate training — fixes the original 6.6 dB PSNR bias.
    """
    def forward(self, pred, ct, mri):
        loss_ct  = F.l1_loss(pred, ct)
        loss_mri = F.l1_loss(pred, mri)
        scale_ct  = (loss_mri.detach() /
                     (loss_ct.detach()  + 1e-8)).clamp(0.5, 2.0)
        scale_mri = (loss_ct.detach()  /
                     (loss_mri.detach() + 1e-8)).clamp(0.5, 2.0)
...
class FusionLoss(nn.Module):
    ...
    Balanced Fusion Loss:
      L = lam_int*L_int + lam_ssim*L_ssim + lam_msssim*L_msssim
          + lam_grad*L_grad + lam_modal*L_modal
    """
    def __init__(self, lam_int=1.0, lam_ssim=0.5, lam_msssim=0.5,
                 lam_grad=0.5, lam_modal=0.3):
        super().__init__()
        self.lam_int    = lam_int
        self.lam_ssim   = lam_ssim
        self.lam_msssim = lam_msssim
        self.lam_grad   = lam_grad
        self.lam_modal  = lam_modal
    ...
    def forward(self, pred, ct, mri):
        ...
        # SSIM: equal weight + extra CT term to counter historical bias
        l_ssim = (
            0.5 * (
                (1.0 - ssim(pred, ct,  data_range=1.0, size_average=True)) +
                (1.0 - ssim(pred, mri, data_range=1.0, size_average=True))
            ) +
            0.3 * (1.0 - ssim(pred, ct, data_range=1.0, size_average=True))
        )
        ...
        total = (self.lam_int    * l_int    +
                 self.lam_ssim   * l_ssim   +
                 self.lam_msssim * l_msssim +
                 self.lam_grad   * l_grad   +
                 self.lam_modal  * l_modal)
...
def train_one_epoch(model, loader, optimizer, criterion, device, epoch):
    ...
    for step, (ct, mri, _) in enumerate(loader):
        ...
        if gate_means:
            gate_balance = (sum((g - 0.5).pow(2) for g in gate_means)
                            / len(gate_means))
        ...
        if step % 50 == 0:
            ...
            if gate_means:
                avg_gate = (sum(g.item() for g in gate_means)
                            / len(gate_means))
...
def validate(model, loader, criterion, device, epoch,
    ...
    rows     = []
    val_loss = 0.0
    metric_keys = ('psnr_ct', 'ssim_ct', 'msssim_ct',
    ...
    # Gate balance report
    gate_vals = [apfm.last_gate_mean.item()
                 for apfm in model.apfm_layers
                 if apfm.last_gate_mean is not None]
    if gate_vals:
        print(f"  APFM gate  "
              f"min={min(gate_vals):.3f}  "
              f"max={max(gate_vals):.3f}  "
              f"avg={sum(gate_vals)/len(gate_vals):.3f}  "
              f"(target ~0.5)")

    fieldnames = ['patient'] + list(metric_keys)
    ...
    with open(csv_path, 'a', newline='') as f:
        ...
        for row in rows:
            writer.writerow(
                {k: (f"{row[k]:.6f}" if k != 'patient' else row[k])
                 for k in fieldnames})
...
def save_grid(ct, mri, pred, metrics: dict,
    ...
    for ci, (label, img, lc) in enumerate(
        ...
        if ci == 2:
            ...
            for line in [
                ...
                clr = ((0, 70, 160) if line.startswith('--')
                       else (5, 5, 110))
...
def save_summary(agg: Dict[str, float], infer_dir: Path,
    ...
    lines = [
        "=" * 58,
        "  CT-MRI FUSION  INFERENCE SUMMARY  (v2 dual-gate APFM)",
        "=" * 58,
...
def do_infer(args, device: torch.device, ckpt_path: str = None):
    ...
    if not ckpt_path:
        raise ValueError(
            "Inference requires --ckpt or a prior training run")
...
def parse_args():
    p = argparse.ArgumentParser(
        description='CT-MRI Fusion Pipeline v2 (dual-gate APFM)')
    ...
    p.add_argument('--ct_ckpt',
                   default='MAEDiff-main/mae_vit_small_patch16_ct/encoder.pth')
    p.add_argument('--mri_ckpt',
                   default='MAEDiff-main/mae_vit_small_patch16_mri/encoder.pth')
    ...
    p.add_argument('--test_dir',
                   default='preprocessing/test',
                   help='test/{patient}/CT/*.png + MR/*.png')
    p.add_argument('--ct_dir',  default=None,
                   help='(legacy) flat CT dir')
    p.add_argument('--mri_dir', default=None,
                   help='(legacy) flat MRI dir')

    # Output: single root for everything
    p.add_argument('--out_dir',
                   default='MAEDiff-main/fusion_output_apfm',
                   help='Root for all outputs: '
                        'checkpoints/, inference/, val_metrics.csv')
