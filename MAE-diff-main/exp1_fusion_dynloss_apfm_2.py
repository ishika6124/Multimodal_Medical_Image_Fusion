...
=======================================================
APFM Change:
  - Channel gate: independent for CT and MRI (each modality gets own GAP → FC → Sigmoid)
  - Spatial gate: kept zero-sum (shared, original behavior)
  - Output: CT_attended + MRI_attended (no zero-sum on channel)
  - Residual: 0.5*(x+y) added at output to prevent signal collapse

Everything else unchanged from original fusion_pipeline.py
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1.  APFM  — Independent channel gates + zero-sum spatial gate
# ─────────────────────────────────────────────────────────────────────────────
class APFM(nn.Module):
    """
    Modified APFM — minimal change from original:

    Channel branch (CHANGED):
      Original: GAP+GMP on concat(CT,MRI) → single shared path → single w_ch
      New:      GAP on CT only  → own FC → Sigmoid → w_ch_CT   (independent)
                GAP on MRI only → own FC → Sigmoid → w_ch_MRI  (independent)

    Spatial branch (UNCHANGED — exactly as original):
      concat(CT,MRI) → Conv → Norm → SiLU → Conv → Norm → Sigmoid → w_sp

    Output (CHANGED — no zero-sum on channel):
      CT_attended  = x * w_ch_CT  * w_sp
      MRI_attended = y * w_ch_MRI * (1 - w_sp)
    ...
    def __init__(self, in_channels: int):
        ...
        # ── CT channel gate (own params, pools CT only) ───────────────────────
        self.ct_gap      = nn.AdaptiveAvgPool2d(1)
        self.ct_ch_conv1 = nn.Conv2d(in_channels, mid, 1)
        self.ct_ch_norm1 = nn.GroupNorm(min(32, mid), mid)
        self.ct_ch_silu  = nn.SiLU()
        self.ct_ch_conv2 = nn.Conv2d(mid, in_channels, 1)
        self.ct_ch_norm2 = nn.GroupNorm(g, in_channels)
        self.ct_ch_sig   = nn.Sigmoid()

        # ── MRI channel gate (own params, pools MRI only) ─────────────────────
        self.mri_gap      = nn.AdaptiveAvgPool2d(1)
        self.mri_ch_conv1 = nn.Conv2d(in_channels, mid, 1)
        self.mri_ch_norm1 = nn.GroupNorm(min(32, mid), mid)
        self.mri_ch_silu  = nn.SiLU()
        self.mri_ch_conv2 = nn.Conv2d(mid, in_channels, 1)
        self.mri_ch_norm2 = nn.GroupNorm(g, in_channels)
        self.mri_ch_sig   = nn.Sigmoid()

        # ── Spatial gate (IDENTICAL to original) ──────────────────────────────
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # ── CT channel gate ───────────────────────────────────────────────────
        w_ch_ct = self.ct_ch_sig(
                      self.ct_ch_norm2(
                          self.ct_ch_conv2(
                              self.ct_ch_silu(
                                  self.ct_ch_norm1(
                                      self.ct_ch_conv1(
                                          self.ct_gap(x)))))))  # (B,C,1,1)

        # ── MRI channel gate ──────────────────────────────────────────────────
        w_ch_mri = self.mri_ch_sig(
                       self.mri_ch_norm2(
                           self.mri_ch_conv2(
                               self.mri_ch_silu(
                                   self.mri_ch_norm1(
                                       self.mri_ch_conv1(
                                           self.mri_gap(y)))))))  # (B,C,1,1)

        # ── Spatial gate (original, zero-sum) ────────────────────────────────
        ...
        w_sp  = self.sigmoid(
                    self.norm4(
                        self.conv4(
                            self.silu2(
                                self.norm3(
                                    self.conv3(xy))))))            # (B,C,H,W)

        # ── Output ───────────────────────────────────────────────────────────
        ct_attended  = x * w_ch_ct  * w_sp
        mri_attended = y * w_ch_mri * (1.0 - w_sp)
        fused        = ct_attended + mri_attended

        # track CT channel gate mean for balance monitoring (target ~0.5)
        self.last_gate_mean = w_ch_ct.mean().detach()

        return fused


# ─────────────────────────────────────────────────────────────────────────────
# 2.  MAE ViT-Small Encoder  (frozen after loading — unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  MAE ViT-Small Decoder  (trainable — unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
def validate(model, loader, criterion, device, epoch,
    ...
    gate_vals = [apfm.last_gate_mean.item()
                 ...
                 if apfm.last_gate_mean is not None]
    if gate_vals:
        print(f"  CT ch-gate  min={min(gate_vals):.3f}  "
              f"max={max(gate_vals):.3f}  "
              f"avg={sum(gate_vals)/len(gate_vals):.3f}  (target ~0.5)")
...
# ─────────────────────────────────────────────────────────────────────────────
# 9.  Inference Utilities  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 10.  Training Entry Point  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 11.  Inference Entry Point  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 12.  Argument Parser + Main  (unchanged)
# ─────────────────────────────────────────────────────────────────────────────
def parse_args():
