...
Change from original:
  - Added LocalTextureLoss to FusionLoss (lam_texture=0.3)
  - Everything else identical to original fusion_pipeline.py

LocalTextureLoss:
  Computes local standard deviation in a 5×5 neighbourhood for both
  pred and target (mean of CT and MRI), then penalises the difference.
  This directly rewards texture richness and prevents the blurry/flat
  outputs that MAE+SSIM losses tend to produce.
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 2.  MAE ViT-Small Encoder  (frozen — unchanged)
# ─────────────────────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────────────────────
# 6.  Loss  — original + LocalTextureLoss added
# ─────────────────────────────────────────────────────────────────────────────
...
# ── NEW: Local Texture Loss ───────────────────────────────────────────────────
class LocalTextureLoss(nn.Module):
    """
    Penalises difference in local texture richness between pred and target.

    For each pixel, computes standard deviation within a k×k neighbourhood
    using the identity:  std = sqrt(E[x²] - E[x]²)

    Applied with avg_pool2d at stride=1 with padding to keep spatial size.
    Target = 0.5*(CT + MRI) — the mean of both modalities.

    Why this helps:
      MAE and SSIM both reward smooth outputs (blurry = low error).
      This loss explicitly penalises smoothness by comparing local std maps,
      forcing the model to reproduce fine-grained texture that increases
      SD and EN in the output.
    """
    ...
    def _local_std(self, x: torch.Tensor) -> torch.Tensor:
        """Compute per-pixel local std in a ks×ks neighbourhood."""
        mu  = F.avg_pool2d(x,    self.ks, stride=1, padding=self.pad)
        mu2 = F.avg_pool2d(x**2, self.ks, stride=1, padding=self.pad)
        # clamp before sqrt to avoid NaN from floating point negatives
...
class FusionLoss(nn.Module):
    """
    Balanced Fusion Loss — original + LocalTextureLoss:
      L = λ_int·L_int + λ_ssim·L_ssim + λ_msssim·L_msssim
          + λ_grad·L_grad + λ_modal·L_modal + λ_texture·L_texture

    L_texture compares local std maps of pred vs mean(CT,MRI).
    λ_texture=0.3 is a safe default — raise to 0.5 if output is still flat.
    """
    def __init__(self, λ_int=1.0, λ_ssim=0.5, λ_msssim=0.5,
                 λ_grad=0.5, λ_modal=0.3, λ_texture=0.3):   # ← new param
        super().__init__()
        self.λ_int     = λ_int
        self.λ_ssim    = λ_ssim
        ...
        self.λ_texture = λ_texture                           # ← new
        self.grad_loss    = GradientLoss()
        self.modal_loss   = DynamicModalityLoss()
        self.texture_loss = LocalTextureLoss(kernel_size=5)  # ← new

    def forward(self, pred, ct, mri):
        """All tensors: (B, 1, 256, 256) in [0,1]."""

        # target for texture loss = mean of both modalities
        target = 0.5 * (ct + mri)                           # ← used by texture

        # L_int: equal MAE vs both modalities (unchanged)
        ...
        # L_texture: local std(pred) vs local std(mean(CT,MRI))  ← NEW
        ...
        total = (self.λ_int     * l_int     +
                 ...
                 self.λ_texture * l_texture)                 # ← new term
...
# ─────────────────────────────────────────────────────────────────────────────
# 8.  Training Utilities  (unchanged except texture logged in comp_str)
# ─────────────────────────────────────────────────────────────────────────────
class EarlyStopping:
...
def parse_args():
    ...
    p.add_argument('--ct_ckpt',
                   default='../mae_vit_small_patch16_ct/encoder.pth')
    p.add_argument('--mri_ckpt',
                   default='../mae_vit_small_patch16_mri/encoder.pth')
