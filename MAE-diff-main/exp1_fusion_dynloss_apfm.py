...
Architecture:
  ...
  - Dual-Gate APFM fusion at every encoder layer (independent CT+MRI gates,
    no zero-sum constraint, full spatial attention, residual connection)
  - MAE ViT-Small decoder (trainable)
  - Fixed Balanced Fusion Loss (DynamicModalityLoss bias corrected)

APFM change from v1:
  OLD: gate = sigmoid(w_ch + w_sp)
       output = x*gate + y*(1-gate)   ← zero-sum, MRI dominates
  NEW: independent ct_channel_gate, mri_channel_gate, ct_spatial_gate,
       mri_spatial_gate → output = proj(ct_attended + mri_attended) + 0.5*(x+y)
       Both modalities can be fully strong simultaneously.
...
Usage:
    # Train only
    python fusion_pipeline_dynloss.py --mode train

    # Inference only (requires --ckpt)
    python fusion_pipeline_dynloss.py --mode infer \
        --ckpt /path/to/best_model.pth \
        --test_dir /path/to/test

    # Train then immediately run inference on test set
    python fusion_pipeline_dynloss.py --mode both
"""
...
# ─────────────────────────────────────────────────────────────────────────────
# 1.  APFM  — Dual Independent Gate (replaces zero-sum v1)
# ─────────────────────────────────────────────────────────────────────────────
class APFM(nn.Module):
    """
    Dual-gate Adaptive Pooling Fusion Module.

    Changes vs original APFM:
      - Independent channel + spatial gates for CT and MRI separately.
        Not zero-sum: CT gaining does NOT force MRI to lose.
      - Spatial attention at full feature resolution (16x16) per modality,
        telling WHERE each modality is informative.
      - Residual from mean input 0.5*(x+y) prevents signal/entropy collapse.
      - last_gate_mean tracks CT spatial gate ratio (target ~0.5).

    Interface unchanged: forward(x, y) -> tensor, same shape.
    """
    ...
    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        ...
        # Apply channel x spatial attention independently — not zero-sum
        ct_attended  = x * ct_ch  * ct_sp
        mri_attended = y * mri_ch * mri_sp

        # Fuse by addition: both can contribute fully
        ...
        # Track CT spatial gate ratio for balance monitoring (target ~0.5)
        self.last_gate_mean = (
...
class DynamicModalityLoss(nn.Module):
    """
    Balanced independent losses — fixes original 6.6 dB PSNR bias.
    Each term scaled by the other's magnitude so neither CT nor MRI dominates.
    """
...
class FusionLoss(nn.Module):
    ...
    def forward(self, pred, ct, mri):
        ...
        # L_ssim: equal weight + extra CT penalty to counter historical bias
        l_ssim = (
            0.5 * (
        ...
        # L_modal: balanced modality loss
...
def save_summary(agg: Dict[str, float], infer_dir: Path,
    ...
    lines = [
        "=" * 58,
        "  CT-MRI FUSION — INFERENCE SUMMARY (dual-gate APFM)",
        "=" * 58,
...
def parse_args():
    p = argparse.ArgumentParser(
        description='CT-MRI Fusion Pipeline (dual-gate APFM)')
